import "server-only"

/**
 * Apple root CA certificates (DER, base64-encoded) used to anchor the x5c
 * certificate chain of StoreKit 2 signed transactions and App Store Server
 * Notifications V2. Embedded (rather than read from disk) so they are always
 * present in the serverless bundle.
 *
 * Source: https://www.apple.com/certificateauthority/
 *  - Apple Root CA - G3 (current StoreKit signing root)
 *  - Apple Root CA - G2
 *
 * Regenerate with:
 *   base64 -w0 AppleRootCA-G3.cer
 */
const APPLE_ROOT_CA_G3_BASE64 =
  ""

const APPLE_ROOT_CA_G2_BASE64 =
  ""

/** DER-encoded Apple root certificates as Buffers for SignedDataVerifier. */
export const appleRootCertificates: Buffer[] = [
  Buffer.from(APPLE_ROOT_CA_G3_BASE64, "base64"),
  Buffer.from(APPLE_ROOT_CA_G2_BASE64, "base64"),
]
