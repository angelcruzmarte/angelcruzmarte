...
// notifications, and the OS media controls) via the Web MediaSession API.
//
 ...
 * - PDFs → the first page rendered to a crisp 512px JPEG data URL,
...
export async function resolveDocumentArtwork(opts: {
  ...
  const { originalUrl, originalMime } = opts
  ...
  try {
    ...
    if (mime.includes("pdf") || /\.pdf(\?|$)/i.test(originalUrl)) {
      ...
      const data = await fetch(originalUrl).then((r) => {
      ...
      // Render at 512px wide — large enough to look sharp on a Lock Screen.
      const scale = 512 / base.width
      ...
      // Fill white first so transparent PDF regions don't render black.
